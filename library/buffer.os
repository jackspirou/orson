...
!  BUFFERING. Do all the work when making a BUFFER type.
     ...
     buffering :−
      (form (list goats, inj limit, type mut base) type tup:
       (if isInt(limit)
        then (if limit ≥ 0
              then (tuple
                     int Start,
              ...
              else error(goats.limit, "non negative inj expression expected")
                   buffering(goats, 0, base))
        else error(goats.limit, "constant expected")
             buffering(goats, 0, base)))
...
!  length. If BASE is omitted then it defaults to CHAR0.
    ...
    do (alt
        ...
        (form (inj limit) type tup:
          buffering(goat(), limit, char0)),
        (form (inj limit, type mut base) type tup:
          buffering(goat(), limit, base))))
