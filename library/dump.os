...
!  WANT must not contain jokers.
      ...
      want :−
       (if isJoked((past want))
        then error($want, "non joker type expected", disp((tuple)))
        else (past want))
      ...
      degree :−
       (if isInt((past degree))
        then (if 0 ≤ (past degree) ≤ (wantSize − MinDispSize) / RowSize
              then (past degree)
              else error($degree, "out of range", 0))
        else error($degree, "constant expected", 0))
...
!  TAG must be a nonnegative constant.
      ...
      tag :−
       (if isInt((past tag))
        then (if (past tag) ≥ 0
              then (past tag)
              else error($tag, "out of range", 0))
        else error($tag, "constant expected", 0))

!  The size of the requested object must be no greater than MAX HUNK SIZE.

     do (if actualSize > MaxHunkSize
         then error($want, "type size too large", nil)
