...
!  the maximum length of a PHRASE, unless we call ENGLISH on LOW(INT) and count
!  the characters in the resulting string.

  type exe Phrase :− buffer(111)    !  Type of BUFFER that holds the string.
...
!  phrase should use commas. For example, this FOR-clause
!
!       (for string phrase in english(low(int2), true) do writeln(phrase))
!
!  writes the string: "minus two billion, one hundred forty seven million, four
!  hundred eighty three thousand, six hundred forty eight" because LOW(INT2) is
!  − 2_147_483_648.
