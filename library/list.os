...
!  LEFT. Return T(T(T(L, R₁), R₂) ..., Rⱼ). The subscripted R's are elements of
!  R. If R is (:), then return L.

  left :−
   (form (form (obj) obj t, obj l, list r) obj:
    (with
      lefting :−
...
!  MAP. Return a list (: T(L₁), T(L₂) ..., T(Lⱼ)) where the subscripted L's are
!  the elements of L.
   ...
   (form (list l, inj b, inj e) list:
    (if ¬ isInt(b)
     then error($b, "constant expected")
          ...
          else if 0 ≤ b ∧ b ≤ e ∧ e ≤ length(l)
               then head(tail(l, b), e − b)
               else error($b, "out of range")
