...
!  SORT. Sort a list of cases C into nondecreasing order of their key strings.
         ...
         sorting :−
          (form (list c) list:
           (if isEmpty(c) ∨ isEmpty(cdr(c))
            then c
            else partitioning((:), c[length(c) / 2], (:), c)))
         partitioning :−
          (form (list l, list m, list r, list c) list:
           (if isEmpty(c)
            then conc(sorting(l), sorting(r))
            else if caar(c) < car(m)
                 then partitioning(cons(car(c), l), m, r, cdr(c))
                 else partitioning(l, m, cons(car(c), r), cdr(c))))
        do sorting(c)))
       ...
       (if isEmpty(c) ∨ isEmpty(cdr(c))
        then false
             ...
             then hasDuplicates(cdr(c))
                  error(car(c), "unexpected element", true)
