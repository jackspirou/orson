...
!  CHECK. Test MAKING's arguments and recover from bad ones. Here G is MAKING's
!  scapegoat list. We test if K is greater than 0, and if T has no jokers.

     check :−
      (alt
       (form (list g, inj k) inj:
        (if isInt(k) ∧ k < 0
         then error(g, "non negative inj expression expected", 0)
         else k)),
       (form (list g, type mut t) type mut:
        (if isJoked(t)
         then error(g, "non joker type expected", void)
         else t)))
...
!  of type REF T to the object.
    ...
    do (alt
        (form (type mut t) foj:
         (with type mut t :− check(goat().t, (past t))
          do (alt
...
!  length of the array. We finally return a pointer of type ROW T to the array.
        ...
        (form (inj k, type mut t) foj:
         (with
           list     g :− goat()
           inj      k :− check(g.k, (past k))
           type mut t :− check(g.t, (past t))
          do (alt
