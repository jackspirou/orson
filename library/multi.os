...
!  assert an error involving the first element of the list G.

     lengthing :−
      (form (list g, inj l) inj:
       (if isInt(l)
        then (if l ≥ 0
              then l
              else error(g, "non negative inj expression expected", 1))
        else error(g, "constant expected", 1)))
...
!  constants in the list L, and whose base type is B.
    ...
    do (alt
        (form (list l, type exe b) type exe:
         (if isEmpty(l)
          then b
          else [lengthing(l, car(l))] [cdr(l)] b)),
     ...
     index :−
      (form (list k) inj:
       (with obj k₀ :− car(k)
        do (if type k₀ ⊆ inj
            then k₀
            else error(k, "inj expression expected", 0))))
    ...
    do (alt
        (form (type [] exe t, inj k) inj:
         (if isInt(k)
          then (if 0 ≤ k < rank(t)
                then lengthing(t, k)
                else error($k, "out of range", 0))
          else error($k, "constant expected", 0))),
        (gen (type [] mut b)
