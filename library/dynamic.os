...
(prog
  ...
  "negative length"  :− except()  !  Array length is less than 0.
...
!  (1) Return a form that returns a REF pointer to an object of type WANT.
     ...
     (gen (type exe base)
       form (type base want) ref base:
        (if isJoked(want)
         then error($want, "non joker type expected", nil)
         else (with temp :− allocate(size(base))
               do (if temp = nil
...
!  array. The array's length is LENGTH, and its base type is WANT.
     ...
     (gen (type exe base)
       form (int length, type base want) row base:
        (if isJoked(want)
         then error($want, "non joker type expected", nil)
         else (with
               ...
               do (if length < 0
                   then (if isInt(length)
                         then error($length, nniee, nil)
                         else throw("negative length"))
                   else (with temp :− allocate(length × size(base))
                         do (if temp = nil
                             then throw("allocator failed")
                             else temp{row base}))))))))
...
!  after which is it automatically released. This form does not work under some
!  GCC compilers and/or some computers.
