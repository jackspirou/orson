...
!  The arrays in LIB.C.RANGES:C are generated automatically by this program. It
!  clones the WCWIDTH function for 21-bit Unicode chars in the LC_ALL locale. A
!  STATE machine figures out where to put delimiters.
  ...
  main :−
   (for int width in 0, 2
    do c's(string, ''setlocale'': c's(int, ''LC_ALL''), ϵ)
       writeln(''struct Range Width%iRanges[] ='': width)
       (with
         var int newWidth :− ⊥
         var int oldFirst :− ⊥
         var int oldLast  :− ⊥
