...
!  MAKE LABEL TREE. Initialize ROOT with a new label tree.
...
!  This is MAKE LABEL TREE's body. Destroy the existing search tree LABEL ROOT,
!  if there is one. Then reset LABEL ROOT to a sentinel node.
...
!  IS DUPLICATE. Test if LABEL is a key in the tree rooted at ROOT. If so, then
!  return true. If not, then add a new node to ROOT that holds a copy of LABEL,
!  and return false.
