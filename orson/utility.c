...
//  IS EFFECTED. Test if TERM might have side effects. Really we test if it has
//  a subterm that is not known to be without side effects. Huh?
...
//  Most hooks have side effects if one or more of their arguments do.
...
//  If its type term is a transformation type, then it's removable.
...
//  If its type term is an execution type, then it's not removable.
...
//  If its type term is a REF pointer type, then it's removable if its value is
//  the constant NIL.
...
//  SORT. Copy an Orson LIST and sort it using an ordering relation implemented
//  as a 2-ary Boolean FORM. Return the sorted copy.

refObject sort(refObject form, refObject list)
{ struct
  { refFrame  link;
    int       count;
    refObject first;
...
//  SORTING. Do all the work for SORT. Destructively sort a list whose 0th tail
//  is FIRST and whose last nonempty tail is LAST. The index of the middle tail
//  is INDEX. We use a divide and conquer algorithm similar to Quicksort.

  void sorting(refRefObject first, refRefObject last, int index)

//  If the list has zero elements, or one element, then it's already sorted, so
//  do nothing.

  { if (d(first) != nil && cddr(d(first)) != nil)
    { int leftCount  = 0;
      int rightCount = 0;
      struct
      { refFrame  link;
        ...
        refObject middle;
        refObject rest;
        refObject rightFirst;
        refObject rightLast;
        refObject temp; } f;
      push(f, 7);

//  If it has more than one element, then find its MIDDLE tail.

      f.middle = d(first);
      while (index > 0)
      { f.middle = cddr(f.middle);
        index -= 1; }

//  Partition the list into disjoint halves. Elements in the left half are less
//  than MIDDLE and elements in right right half are not, according to FORM. We
//  don't include MIDDLE in either half. If we cannot apply FORM to an element,
//  then we assert an error, and discard the element. If FORM returns an object
//  that's not an integer constant, then we pretend it returned 1.

      f.rest = d(first);
      while (f.rest != f.middle)
      { if (
         isApplied(toss, r(f.temp), form, 2,
          car(f.rest),   cadr(f.rest),
          car(f.middle), cadr(f.middle)))
        { if (isIntegerNonzero(f.temp))
          { leftCount += 1;
            if (f.leftFirst == nil)
            { f.leftFirst = f.leftLast = f.rest;
              f.rest = cddr(f.rest); }
            else
            { f.temp = cddr(f.rest);
              f.leftLast = (cddr(f.leftLast) = f.rest);
              f.rest = f.temp; }}
          else
          { rightCount += 1;
            if (f.rightFirst == nil)
            { f.rightFirst = f.rightLast = f.rest;
              f.rest = cddr(f.rest); }
            else
            { f.temp = cddr(f.rest);
              f.rightLast = (cddr(f.rightLast) = f.rest);
              f.rest = f.temp; }}}
        else
        { objectError(f.rest, elementErr);
          f.rest = cddr(f.rest); }}
      f.rest = cddr(f.rest);
      while (f.rest != nil)
      { if (
...
//  Recursively sort both halves. Concatenate the sorted left half, followed by
//  MIDDLE, followed by the sorted right half. Clean up and return.

      if (f.rightFirst == nil)
      { cddr(f.leftLast) = nil;
        sorting(r(f.leftFirst), r(f.leftLast), leftCount / 2);
        d(first) = f.leftFirst;
        d(last) = f.leftLast;
        cddr(f.leftLast) = f.middle;
        cddr(f.middle) = nil; }
      else if (f.leftFirst == nil)
           { cddr(f.rightLast) = nil;
             sorting(r(f.rightFirst), r(f.rightLast), rightCount / 2);
             d(first) = f.middle;
             cddr(f.middle) = f.rightFirst;
             d(last) = f.rightLast; }
           else
           { cddr(f.leftLast)  = nil;
             cddr(f.rightLast) = nil;
...
//  Lost? This is SORT's body. If LIST is empty, or if it has one element, then
//  it's already sorted.
...
//  Otherwise we copy LIST and destructively sort the copy. COUNT is the number
//  of pairs in the list, not the number of tails, so we must divide by four to
//  get the index of the middle tail.

  else
  { int count = 1;
    f.first = f.last = makePaire(car(list), nil, info(list));
    list = cdr(list);
    while (list != nil)
    { count += 1;
      f.last = (cdr(f.last) = makePaire(car(list), nil, info(list)));
      list = cdr(list); }
    sorting(r(f.first), r(f.last), count / 4); }
