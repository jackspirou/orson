...
//  IS COTYPE. Here LEFT TYPE and RIGHT TYPE are strongly ground runtime types.
//  Test if LEFT TYPE coerces to RIGHT TYPE.
...
//  are strongly ground runtime types. If RIGHT TYPE is NIL, then simply return
//  LEFT TYPE (this helps compute an upper bound on a series of types).  If one
//  type coerces to the other, then return the type to which it coerces.  If no
//  coercions are possible, then return VOID.

refObject supertype(refObject leftType, refObject rightType)
{ if (rightType == nil)
  { return leftType; }
  else if (isCotype(leftType, rightType))
       { return rightType; }
       else if (isCotype(rightType, leftType))
            { return leftType; }
            else
            { return voidSimple; }}
