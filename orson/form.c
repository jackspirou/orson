...
//  If our ALTS type has exactly one member, then we did not need an ALTS type.
//  Clean up and return.
...
//  is used to implement hooks that apply forms passed to them as arguments.
...
//  corresponding type from LEFT PARS, so as to satisfy the continuation ETC.
    ...
    { if (
       isSubtyping(
        ({ bool lambda()
           ...
           lambda; }),
        rightLayer, f.rightType, leftLayer, f.leftType))
      { found = true;
        going = false; }
...
//  Try coercing CHAR0 to CHAR1. If it fails, then fall through to next case.
                ...
                ({ bool lambda()
                   { return
                      isSubsuming(etc,
                       ...
                       rightLayer, cddr(rightPars)); }
                   lambda; }),
                plainLayer, char1Simple, leftLayer, f.leftType);
              if (found)
              { going = false;
                break; }}

//  Try coercing CHAR1 to INT2. If we fell through from the previous case, then
//  try coercing CHAR0 to INT2 instead.
                ...
                ({ bool lambda()
                   { return
                      isSubsuming(etc,
                       ...
                       rightLayer, cddr(rightPars)); }
                   lambda; }),
                plainLayer, int2Simple, leftLayer, f.leftType);
              going = false;
              break; }

//  Try coercing INT0 to INT1. If it fails, then fall through to the next case.
                ...
                ({ bool lambda()
                   { return
                      isSubsuming(etc,
                       ...
                       rightLayer, cddr(rightPars)); }
                   lambda; }),
                plainLayer, int1Simple, leftLayer, f.leftType);
              if (found)
              { going = false;
                break; }}

//  Try to coerce INT1 to INT2. If we fell through from the previous case, then
//  try to coerce INT0 to INT2 instead.
                ...
                ({ bool lambda()
                   { return
                      isSubsuming(etc,
                       ...
                       rightLayer, cddr(rightPars)); }
                   lambda; }),
                plainLayer, real1Simple, leftLayer, f.leftType);
              going = false;
              break; }

//  Try coercing REF T to ROW T.
                ...
                ({ bool lambda()
                   { return
                      isSubsuming(etc,
                       ...
                       rightLayer, cddr(rightPars)); }
                   lambda; }),
                plainLayer, f.rightType, leftLayer, f.leftType);
              going = false;
              break; }

//  Try to coerce VAR T to its base type T. Do a tail recursion, in case T must
//  be coerced again.
...
//  If we get here, then no coercions are possible.
