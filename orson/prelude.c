...
//  aren't bound to. (See ORSON/HUNK.)
  ...
  indJoker = makeJoker("ind", char0Hook,   char1Hook,   int0Hook,
                              int1Hook,    int2Hook);
  ...
  symJoker = makeJoker("sym", symHook);
...
//  Bind names to most jokers.
  ...
  bindName("ind", makePrefix(typeHook, indJoker), indJoker);
  ...
  bindName("sym", makePrefix(typeHook, symJoker), symJoker);
  bindName("tra", makePrefix(typeHook, traJoker), traJoker);
  bindName("tup", makePrefix(typeHook, tupJoker), tupJoker);

//  Make type types. The garbage collector must mark them all.
  ...
  typeSymJoker     = makePrefix(typeHook, symJoker);
...
//  Bind quoted names to codes for errors that may be asserted by the user.
  ...
  bindError("transformation assertion failed",      assertErr);
