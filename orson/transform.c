  ...
  { term = car(terms);
    ...
    { level += 1;
      ...
      { objectError(terms, haltErr);
        objectError(terms, formsTooDeepErr);
        compiling = false;
        unlinking = false;
        longjmp(halt, true); }
...
//  LAYER (see ORSON/APPLY), then we can apply it. If we can't apply any of the
//  member forms, then we have an error.
  ...
  { f0.value = cdr(f0.value);
    ...
    { if (f0.value == nil)
      ...
      { f0.type  = car(f0.value); f0.value = cdr(f0.value);
        f0.layer = car(f0.value); f0.value = cdr(f0.value);
        f0.body  = car(f0.value); f0.value = cdr(f0.value);
        f0.layer = pushLayer(f0.layer, plainInfo);
        if (isApplicable(f0.types, f0.values, f0.layer, f0.type))
...
//  is VOID. In that case, we coerce TYPE and VALUE to VOID.
        ...
        { destroyPairs(f0.types);  f0.types = nil;
          ...
          f0.type = cdr(degen(f0.type));
          f0.yield = cadr(f0.type);
          f0.goatArgs = oldGoatArgs; f0.goatPars = oldGoatPars;
          oldGoatArgs = goatArgs;    oldGoatPars = goatPars;
          goatArgs    = f0.term;     goatPars    = car(f0.type);
          f0.layers = layers;
          layers = f0.layer;
          transform(r(f0.type), r(f0.value), f0.body);
          if (! isCoerced(r(f0.type), r(f0.value), layers, f0.yield))
          { if (isSubtype(layers, f0.yield, plainLayer, voidSimple))
            { if (isGroundSubtype(f0.type, exeJoker))
              { f0.value = makeVoidCast(f0.value); }
              else
              { objectError(f0.body, exeErr);
                f0.value = skip; }}
            else
            { objectError(f0.body, typeErr);
              f0.value = skip; }
            f0.type = voidSimple; }
          layers = f0.layers;
          goatArgs    = oldGoatArgs; goatPars    = oldGoatPars;
          oldGoatArgs = f0.goatArgs; oldGoatPars = f0.goatPars;
          break; }
...
//  a WITH that binds a name N to A, so it will be protected.
//
...
//  may be simplified away as shown.
//
//    ?toVar(?rowTo(A, T))  =>  A
//    ?toVar(?varTo(A, T))  =>  A
//
...
//  Or to apply something that's neither a FORM or a PROC.
...
{ struct
  ...
  { if (toInteger(f.length) >= 0)
    ...
    { objectError(cdr(goatArgs), rangeErr);
      f.length = integerZero; }}
  else
  { objectError(cdr(goatArgs), constantErr);
    f.length = integerZero; }
...
//  TRANSFORM LABEL. Transform the non NONE label at the start of TERMS and set
//  LABEL to the result. LABELS is a list of constants that previously appeared
//  in labels, which we use to detect duplicates. LABELS might contain NONE.
  ...
  { f.temp = car(f.terms);
    ...
    { transform(r(f.type), r(f.next), f.temp);
      if (isGroundCoerced(r(f.type), r(f.next), f.indexType))
      { if (isCharacter(f.next) || isInteger(f.next))
      ...
      { objectError(f.temp, labelTypeErr); }
      f.temp = cdr(f.temp); }}

//  TRANSFORM NONE. Transform the NONE label at the start of TERMS.
...
//  Transform the index term. It selects the clause we want.

  push(f, 13);
  transform(r(f.indexType), r(f.indexValue), terms);
  if (! isGroundCoerced(r(f.indexType), r(f.indexValue), indJoker))
  { objectError(terms, chaInjErr);
    f.indexType = voidSimple;
    f.indexValue = skip; }

//  If INDEX VALUE is a constant, then search for an expression with a matching
//  label, and transform it. First, search expressions with non NONE labels and
//  transform their labels as we go. We don't care which ones are exceptional.

  f.terms = cdr(terms);
  if (isCharacter(f.indexValue) || isInteger(f.indexValue))
  { while (f.terms != nil)
    { if (car(f.terms) == hooks[noneHook])
      ...
      { transformLabel();
        f.terms = cdr(f.terms);
        if (isMember(f.indexValue, f.label))
        { transform(r(f.super), r(f.first), f.terms);
...
//  Transform all remaining non NONE labels, in case they have errors.
...
//  Transform NONE labels. The ones after the first are errors. If there was no
//  matching label above, then transform the first NONE labeled expression that
//  we find. That gives us the result of the CASE.

    f.terms = cdr(terms);
    while (f.terms != nil && f.super == nil)
    { if (car(f.terms) == hooks[noneHook])
      { transformNone();
        f.terms = cdr(f.terms);
        transform(r(f.super), r(f.first), f.terms);
...
//  If there were no matching labels, and no NONEs, then the CASE becomes SKIP.

    if (f.super == nil)
    { f.super = voidSimple;
      f.first = skip; }}

//  If INDEX VALUE is not a constant, then build a new CASE by transforming all
//  expressions and their labels. Transform non NONE labeled expressions first.
//  SUPER is the supertype of all non exceptional expressions.

  else
  { f.last = makePair(f.indexValue, nil);
    ...
    { if (car(f.terms) == hooks[noneHook])
      ...
      { transformLabel();
        ...
        f.terms = cdr(f.terms);
        transform(r(f.type), r(f.next), f.terms);
...
//  Then we transform NONE labeled expressions and put the first one at the end
//  of the CASE. Any additional NONE labeled expressions are errors.
...
//  If no expressions were labeled NONE, then use SKIP as the final expression.
//  If all expressions were exceptional, then their supertype is VOID.

    if (f.type == nil)
    { f.super = supertype(voidSimple, f.super);
      cdr(f.typesLast) = makePair(voidSimple, nil);
      cdr(f.last) = makePair(skip, nil); }
    if (f.super == nil)
    { f.super = voidSimple; }

//  Here the CASE's type is SUPER. If SUPER is VOID, then we must cast the CASE
//  to VOID so it won't return a value. If SUPER is another execution type then
//  revisit all nonexceptional clauses and coerce them to SUPER. If SUPER isn't
//  an execution type, then the CASE turns to SKIP (we've asserted errors about
//  its exceptions already).
...
//  CELL, CHARs, INTs, JOKER, LIST, NULL, REALs, VOID.  Jokers and simple types
//  transform to themselves. The parser can make untransformed terms with these
//  types in them (see ORSON/LOAD).
...
{ struct
  ...
  { objectError(cddr(goatArgs), nonJokerErr);
...
{ struct
  ...
  if (isCharacter(f.left) && isCharacter(f.right))
...
//  ERR. Assert an error in a form call. We interact with APPLY in a kludgy way
//  by using OLD GOAT ARGS. This is a simpler version of SYM ERR.
...
{ struct
  { refFrame  link;
    ...
    refObject err; } f;
  push(f, 1);
  transform(toss, r(f.err), terms);
  if (oldGoatArgs == nil)
  { objectError(goatArgs, notInsideFormErr); }
  else if (isInteger(f.err))
       { int err = toInteger(f.err);
         if (isInSet(err, userErrs))
         { objectError(oldGoatArgs, err); }
         else
         { objectError(cdr(goatArgs), errNumberErr); }}
       else
       { objectError(cdr(goatArgs), constantErr); }
  pop();
  d(type) = voidSimple;
...
//  GEN. Transform a FORM type, with one or more GEN prefixes. We must do it in
//  a way that forms applied during transformation can never see generic names,
//  but the generic names will still appear in the transformed type. We do this
//  in a way that is similar to removing quantified variables by Skolemization.
//  We replace generic names by Skolem types before transformation, and replace
//  Skolem types with stubs after transformation. We use stubs to avoid capture
//  of generic names. Kids, don't try this at home!
...
{ struct
  ...
  transform(r(f.rightType), r(f.right), cdr(terms));
  if (isIntegerZero(f.right))
  { objectError(cddr(goatArgs), divideByZeroErr);
    f.type = int0Simple;
    f.value = integerZero; }
    ...
    { if (begin < end)
      ...
      { f.first = f.last = makeTriple(hooks[lastHook], nil, nil);
        ...
        finishLast(r(f.first), r(f.last), voidSimple, skip); }}
         ...
         { objectError(goatArgs, rangeErr);
           f.first = skip; }}
  else
  { if (! isInteger(f.begin))
    { objectError(cdr(goatArgs), constantErr); }
    if (! isInteger(f.end))
    { objectError(cddr(goatArgs), constantErr); }
    if (! isInteger(f.step))
    { objectError(cdddr(goatArgs), constantErr); }
    f.first = skip; }
  pop();
  d(type) = voidSimple;
  d(value) = f.first;
...
{ struct
  ...
  { if (f.value == nil)
    { objectError(cdr(goatArgs), tooFewElemsErr);
      f.type = voidSimple;
      f.value = skip; }
    else
    { f.type = car(f.value);
  ...
  { objectError(cdr(goatArgs), elementErr);
    f.type = voidSimple;
    f.value = skip; }
  pop();
  d(type) = f.type;
...
{ struct
  ...
  { if (isInSet(toInteger(f.err), userErrs))
    ...
    { objectError(cddr(goatArgs), errNumberErr); }}
  else
  { objectError(cddr(goatArgs), constantErr); }
  pop();
  d(type) = voidSimple;
...
//  LIST GOAT. Return the scapegoat list for the form that called the form that
//  called this hook. We interact with APPLY in a kludgy way, via OLD GOAT ARGS
//  and OLD GOAT PARS.
...
//  If no form is being applied right now, then we have an error, so return the
//  empty list.
...
//  The first element in the list is the joker SYM. It represents the form that
//  is being applied.

  else
  { f.args = oldGoatArgs;
    f.last = makePaire(symJoker, nil, info(f.args));
    f.first = makePaire(typeSymJoker, f.last, info(f.args));
    f.args = cdr(f.args);

//  Each remaining element denotes a parameter name. The element $N denotes the
//  name N. The joker SYM denotes the missing name NO NAME.

    f.pars = oldGoatPars;
    while (f.args != nil)
    { f.pars = cdr(f.pars);
      if (car(f.pars) == noName)
      { f.value = symJoker;
        f.type = typeSymJoker; }
      else
      { f.value = makePrefix(symHook, car(f.pars));
        f.type = makePrefix(typeHook, f.value); }
      f.last = (cdr(f.last) = makePaire(f.type, nil, info(f.args)));
      f.last = (cdr(f.last) = makePaire(f.value, nil, info(f.args)));
      f.args = cdr(f.args);
      f.pars = cdr(f.pars); }}
...
//  LIST MEM. Return the first tail of a list whose CAR is a subtype of a given
//  type. It is an error if no such tail exists.
  ...
  { if (f.value == nil)
    { objectError(cdr(goatArgs), tooFewElemsErr);
      break; }
          ...
          isGroundSubtype(car(f.value), typeObjJoker) &&
          isGroundSubtype(cadr(f.value), f.type))
         { break; }
         else
         { f.value = cddr(f.value); }}
  pop();
...
{ struct
  ...
  push(f, 2);
  transform(toss, r(f.form), terms);
  transform(toss, r(f.list), cdr(terms));
  f.list = sort(f.form, f.list);
  pop();
  d(type) = listSimple;
  d(value) = f.list;
  break; }

//  LIST SUB. Return a sublist of a list. Its indexes are greater than or equal
//  to START and less than END. Both START and END must be integer constants.
//
//    ?listSub(l, 0, ?listLen(l))  =>  l
//    ?listSub(l, r, r)            =>  (:)
//
//  If START and END both have reasonable values, then we compute the LENGTH of
//  the sublist, skip the first START elements from the original list, and copy
//  the next LENGTH elements.
...
{ struct
  ...
  { int start = toInteger(f.start);
    int end = toInteger(f.end);
    if (0 <= start && start <= end && end <= countPairs(f.objects))
    { int length = 2 * (end - start);
      while (start > 0 && f.objects != nil)
    ...
    { objectError(cddr(goatArgs), rangeErr);
      objectError(cdddr(goatArgs), rangeErr); }}
  else
...
//  be a value term. It's transformed to RIGHT TYPE (which must be a subtype of
...
//  we also replace LEFT TYPE by RIGHT TYPE.
         ...
         { transform(r(f.rightType), r(f.rightValue), f.terms);
           isGroundCoerced(r(f.rightType), r(f.rightValue), mutJoker);
           if (isGroundSubtype(f.rightType, f.leftType))
...
{ struct
  ...
  { objectError(cddr(goatArgs), nonJokerErr);
    ...
    f.value = hooks[nilHook]; }
       ...
       { objectError(cddr(goatArgs), nonNullTypeErr);
         f.newType = nullSimple;
         f.value = hooks[nilHook]; }
       else if (isForwarding(f.newType) || isForwarding(f.oldType))
            { if (isForwarding(f.newType))
              { objectError(cddr(goatArgs), nonForwardErr); }
              if (isForwarding(f.oldType))
              { objectError(cdr(goatArgs), nonForwardErr); }
              f.newType = nullSimple;
              f.value = hooks[nilHook]; }
  pop();
  d(type) = f.newType;
...
{ struct
  ...
  { objectError(goatArgs, divideByZeroErr);
    f.type = int0Simple;
    f.left = integerZero; }
...
{ struct
  ...
  transform(toss, r(f.newType), cdr(terms));
  if (isNil(f.value))
  { objectError(cdr(goatArgs), nonNilErr);
    f.newType = voidSimple;
    f.value = skip; }
  else if (isGroundSubtype(f.oldType, nullSimple))
       { objectError(cdr(goatArgs), nonNullTypeErr);
         f.newType = voidSimple;
         f.value = skip; }
...
{ struct
  ...
  { if (bytes(toRefString(f.slot)) == 0)
    { objectError(cdddr(goatArgs), objectErr);
      f.type = voidSimple;
      f.value = skip; }
    else
...
{ struct
  ...
  { if (bytes(toRefString(f.value)) == 0)
    { objectError(cddr(goatArgs), objectErr);
      f.type = voidSimple;
      f.value = skip; }
    else
    { f.value = makePair(f.value, nil);
...
//  STR VER. If the constant string argument is not equal to VERSION NAME, then
//  report an error and HALT transformation. If there is no such argument, then
//  simply return VERSION NAME as an Orson string. (See ORSON/GLOBAL.)
...
{ struct
  ...
  push(f, 1);
  if (terms == nil)
  { f.string = bufferToString(versionName); }
  else
  { transform(toss, r(f.string), terms);
    if (isString(f.string))
    { char buffer[strlen(versionName) + 1];
      stringToBuffer(buffer, toRefString(f.string));
      if (strcmp(versionName, buffer) != 0)
      { objectError(goatArgs, haltErr);
        objectError(cdr(goatArgs), versionErr);
        compiling = false;
        unlinking = true;
        longjmp(halt, true); }}
    else
...
{ struct
  { refFrame  link;
    ...
    refObject value; } f;
  push(f, 2);
  f.value = makePrefix(symHook, car(terms));
  f.type = makePrefix(typeHook, f.value);
  pop();
  d(type) = f.type;
  d(value) = f.value;
...
//  SYM ERR. Assert an error in an argument to a form call. We communicate with
//  APPLY in a kludgy way by using OLD GOAT ARGS and OLD GOAT PARS.
...
//  Transform the arguments. SYMBOL is a symbol type that specifies a parameter
//  name. It corresponds to an argument with the error number ERR.
...
//  If no form is being applied right now, then we have an error.

  if (oldGoatArgs == nil)
  { objectError(goatArgs, notInsideFormErr); }

//  Here ERR is a constant integer that represents an error. Search in OLD GOAT
//  PARS for a parameter name matching the name in SYMBOL. If we find one, then
//  we assert ERR for the corresponding argument in OLD GOAT ARGS. Otherwise we
//  have an error.

  else if (isInteger(f.err))
       { int err = toInteger(f.err);
         if (isInSet(err, userErrs))
         { f.args = cdr(oldGoatArgs);
           f.pars = oldGoatPars;
           f.temp = makePrefix(symHook, nil);
           while (true)
           { if (f.args == nil)
             { objectError(cdr(goatArgs), noSuchParamErr);
               break; }
             else
             { f.pars = cdr(f.pars);
               cadr(f.temp) = car(f.pars);
               if (isGroundSubtype(f.temp, f.symbol))
               { objectError(f.args, err);
                 break; }
               else
               { f.args = cdr(f.args);
                 f.pars = cdr(f.pars); }}}}

//  If ERR isn't a constant error number, then we have an error.
...
//  TUPLES. Return a type that matches a TUPLE type with a given slot.
...
{ struct
  ...
  push(f, 3);
  transform(toss, r(f.type), terms);
  transform(toss, r(f.name), cdr(terms));
  f.value = makePair(f.name, nil);
  f.value = makePair(f.type, f.value);
...
//  FORM, or GEN type.
  ...
  { case altsHook:
    ...
    { f.last = (cdr(f.last) = makePair(f.left, nil));
      break; }
    default:
    { objectError(cdr(goatArgs), nonJokerErr);
      break; }}
...
{ struct
  ...
  { case arrayHook:
    ...
    { if (isForwarded(f.value))
      ...
      break; }
    default:
    { objectError(cdr(goatArgs), noBaseTypeErr);
      f.type = typeVoid;
      f.value = voidSimple;
      break; }}
  pop();
...
//  LEFT, then we have an error.
           ...
           { f.type = car(f.right); f.right = cdr(f.right);
             ...
             { objectError(cddr(goatArgs), repeatedNameErr);
               f.name = noName; }
             f.last = (cdr(f.last) = makePair(f.type, nil));
             f.last = (cdr(f.last) = makePair(f.name, nil)); }
           internSize(1, f.first); }}
...
{ struct
  ...
  transform(toss, r(f.left), terms);
  transform(toss, r(f.right), cdr(terms));
  f.right = toBool[isCotype(f.left, f.right)];
  pop();
  d(type) = int0Simple;
  d(value) = f.right;
  break; }
...
//  slots. The slot is specified by a SYM type. (See ORSON/SIZE.)
