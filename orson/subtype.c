...
//  REF or ROW type. We implement this by having the SKO HOOK case fall through
//  into the cases for REFER HOOK and ROW HOOK.
...
//  Test if a SYMbol type LEFT TYPE is a subtype of RIGHT TYPE.
        ...
        { switch (toHook(car(rightType)))
          { case jokerHook:
            ...
            case symHook:
            { return cadr(leftType) == cadr(rightType) && etc(); }
            default:
            { return false; }}
          break; }
