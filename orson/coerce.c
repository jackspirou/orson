...
//  Try to coerce a CHAR0 to a CHAR1. If that didn't work, then fall through to
//  the next case. In this and later cases, we don't need a layer for LEFT TYPE
//  because it's always strongly ground.
...
//  Try to coerce an INT0 to an INT1. If that didn't work, then fall through to
//  the next case.
...
//  If we get here, then no coercions are applicable.

      default:
      { return false; }}}

//  If we get here, then no more coercions are necessary. Clean up and return.
