...
//  WRITE BLANKS. Write COUNT blanks directly to STREAM.

void writeBlanks(refStream stream, int count)
{ while (count > 0)
  { fputc(' ', stream);
    count -= 1; }}
...
//  WRITE HERALD. Write a winged comment to STREAM giving the name of the Orson
//  compiler, its version, the current date, and the current time. Depending on
//  the date, and the values of ME and VERSION, it might look like this.
//
