...
//  Hooks. These must be made in alphabetical order.
  ...
  makeHook("listGoat",    listGoatHook);
  makeHook("listLen",     listLenHook);
  makeHook("listMake",    listMakeHook);
  makeHook("listMem",     listMemHook);
...
//  sets may contain tokens that can't legally appear after parsing.
  ...
  colonTermSet           = makeSet(boldFormToken, boldGenToken, boldNoneToken,
                            boldProcToken, colonToken, dollarToken, endToken,
                            nameToken, openBraceToken, openBracketToken,
                            openParenToken, prefixToken, simpleToken, sumToken,
                            sumPrefixToken);
  ...
  termSet                = makeSet(boldFormToken, boldGenToken, boldNoneToken,
                            boldProcToken, dollarToken, nameToken,
                            openBraceToken, openBracketToken, openParenToken,
                            prefixToken, simpleToken, sumToken,
                            sumPrefixToken);
  unitSet                = makeSet(boldNoneToken, dollarToken, nameToken,
                            simpleToken, openParenToken); }
...
//  NEXT CLOSE BRACKET. Parse "]".

  void nextCloseBracket()
  { tokenEndsTerm = true;
    token = closeBracketToken;
    nextChar(); }

//  NEXT CLOSE PAREN. Parse ")".
...
//  NEXT INFIX. Parse a general infix or update operator.
...
//  NEXT JUNCTION. Parse a conjunction or disjunction operator.

  void nextJunction(int infixToken)
  { tokenEndsTerm = false;
    appendChar(r(tokenStringEnd), ch);
    d(tokenStringEnd) = eosChar;
    nextChar();
...
//  NEXT OPEN BRACKET. Parse "[".

  void nextOpenBracket()
  { tokenEndsTerm = false;
    token = openBracketToken;
    nextChar(); }

//  NEXT OPEN PAREN. Parse "(".
...
//  NEXT POSTFIX. Parse a general postfix operator.
...
//  NEXT PREFIX. Parse a general prefix operator.
...
//  NEXT RELATION. Parse a general relational operator.
      ...
      { case 0x00:
        { switch (0xFF & ch)
          { case 0x00: { nextEndOfLine();           break; }
            ...
            case '{':  { nextOpenBrace();           break; }
            case '|':  { nextInfix(sumToken);       break; }
            case '}':  { nextCloseBrace();          break; }
            case '~':  { nextInfix(sumPrefixToken); break; }
        ...
        { switch (0xFF & ch)
          { case 0x00: { nextName();                 break; }  // forall
            ...
            case 0x27: { nextJunction(boldAndToken); break; }  // wedge
            case 0x28: { nextJunction(boldOrToken);  break; }  // vee
...
//  Parse a symbol type.
        ...
        { int firstCount;
          ...
          { sourceError(nameErr);
            f1.first = makeStub(nil); }
          f1.first = makePaire(f1.first, nil, lastCount);
          f1.first = makePaire(hooks[symHook], f1.first, firstCount);
          break; }
